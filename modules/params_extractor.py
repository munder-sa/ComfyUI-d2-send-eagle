...
def dprint(str, dict=[]):
    if DEBUG:
        print(f"debug:{str}")  # noqa: T201
        if dict:
            print(json.dumps(dict, indent=2))  # noqa: T201
...
class ParamsExtractor:
    def __init__(
        ...
        self._load_config()
        self.gen_info: TGenInfo = self._gather_info(
    ...
    def _extract_image_size(self, node):
        ...
        width, height = self._get_image_size_from_latent_image(
            node["inputs"]["latent_image"][0]
        )
        return width, height
    ...
    def _get_image_size_from_latent_image(self, node_number):
        ...
        if "clip_width" in target_node.get("outputs", {}):
            return target_node["outputs"]["clip_width"], target_node["outputs"][
                "clip_height"
            ]
